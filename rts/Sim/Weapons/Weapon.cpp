...
bool CWeapon::TryTargetRotate(const CUnit* unit, bool userTarget, bool manualFire)
{
	...
	if (gs->frameNum == 9678) {
		LOG("CWeapon::TryTargetRotate(9678) Owner:[%s %p] Target:[%s %p] tempTargetPos(%u, %u, %u) mainDir(%u, %u, %u) aimFromPos(%u, %u, %u) weaponHeading(%d) enemyHeading(%d)", \
			this->owner->unitDef->name.c_str(), this->owner, \
			unit->unitDef->name.c_str(), unit, \
