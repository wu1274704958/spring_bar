...
CUnitDrawerData::CUnitDrawerData()
	: CEventClient("[CUnitDrawerData]", 271828, false)
{
	eventHandler.AddClient(this);

	for (auto& mr : opaqueModelRenderers) { mr.Init(); }
	for (auto& mr : alphaModelRenderers ) {	mr.Init(); }
...
CUnitDrawerData::~CUnitDrawerData()
{
	...
	for (auto& mr : opaqueModelRenderers) { mr.Kill(); }
	for (auto& mr : alphaModelRenderers)  { mr.Kill(); }
...
void CUnitDrawerData::RenderUnitCreated(const CUnit* unit, int cloaked)
{
	...
	if (unit->model != nullptr) {
		if (cloaked) {
			alphaModelRenderers[MDL_TYPE(unit)].AddObject(unit);
		}
		else {
			opaqueModelRenderers[MDL_TYPE(unit)].AddObject(unit);
		}
	}

	UpdateUnitMiniMapIcon(unit, false, false);
	unsortedUnits.push_back(u);
}
